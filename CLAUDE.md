...
**Never curl the console's OWN lisply port from inside a `lisp_eval`** (2026-09-09): `(lisply-shell-bounded "curl http://localhost:7080/lisply/...")` deadlocks — curl waits for the httpd to answer, and the httpd runs on the event loop that is blocked waiting for curl. The bound does eventually fire, but the transport is dark until then. Probe the console's endpoints with `lisply-shell-async` (the child runs beside the loop; read its output on a later call), from another room, or from the host.
